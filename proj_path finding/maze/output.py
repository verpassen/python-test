...
class app():
	...
	def create_widget(self):
		...
		B3.bind("<Button-1>",self.consolid_maze)
	...
	def consolid_maze(self,event):
		print('print maze info.')
		self.Matrix_maze=np.zeros((self.n,self.n))
		for k in range(len(self.Maze_pos)):
			x , y = int(self.Maze_pos[k][0]-1),int(self.Maze_pos[k][1]-1)	 
			self.Matrix_maze[y][x] = 5 	
		# the x and y dir. is opposite from the definition of the 
		# row and column in matrix
		# that's why here is [y][x] , but not [x][y]
		#print(self.Matrix_maze)
